...
final readonly class SpaceLanguagesRemoveAction
{
     ...
     */
    public function execute(string $spaceId, array $languages): SpaceLanguagesRemoveResult
    {
        ...
        foreach ($existingLanguages as $language) {
            $code = $language['code'];
            if (in_array($code, $desiredLanguages, true)) {
                $removedLanguages[] = $code;
                continue;
            }
        ...
        if ($removedLanguages === []) {
            return new SpaceLanguagesRemoveResult(false, $this->languageCodesFromOptions($existingLanguages), []);
        }

        $options = is_array($space['options'] ?? null) ? $space['options'] : [];
        $options['languages'] = $remainingLanguages;
        ...
        $stillPresentLanguages = array_values(array_intersect($desiredLanguages, $confirmedLanguages));
        if ($stillPresentLanguages !== []) {
            throw new \RuntimeException('Failed to configure space languages: Storyblok did not remove language(s): ' . implode(', ', $stillPresentLanguages));
        }

        return new SpaceLanguagesRemoveResult(true, $this->languageCodesFromOptions($remainingLanguages), $removedLanguages);
    }
